...
| --- | --- | --- | --- |
| `src/chrono.ts` | `src/Chrono.php`, `src/ChronoEngine.php`, `src/Configuration.php`, `src/ConfiguredChronoEngine.php` | Partial | PHP now has a configuration-backed engine runner used by all current locale engines. The default English engine is exposed through `Chrono::english()` and the upstream-shaped `Chrono::en()` alias, with public parse-date entrypoint coverage returning Carbon instances. Source-style cloning is exposed through `clone()`, while upstream mutable parser/refiner-array customization maps to immutable `withParser()`, `withRefiner()`, `withoutParser()`, and `withoutRefiner()` methods. Parser results now sort by start index while preserving parser order for same-index ties like upstream. Upstream system-test replacement examples are covered for slash-date parser replacement and English casual-relative parser option replacement. |
| `src/results.ts` | `src/ParsedResult.php`, `src/ParsedComponents.php`, `src/Reference.php` | Partial | Carbon-backed results exist. Parsed components now keep separate known and implied date/time values, delete both value sets like upstream, default empty component time to implied noon like upstream `ParsingComponents`, and preserve explicitly assigned values for validity checks before Carbon normalization, including upstream-style hour/minute/second/millisecond bounds, proleptic month/day validation for before-common-era years, and rejection of local times normalized through a DST gap. Constructor-provided integer component values now rebuild the backing Carbon date like upstream's component-derived `date()` behavior, while PHP-only boolean certainty markers are normalized from the provided Carbon date values. Result tags are component-derived like upstream; the PHP constructor still accepts tags as a convenience and immediately applies them to components. Component and result cloning now mirror upstream by copying structural values without carrying tags, while merge helpers explicitly re-add source tags. Component and result debug strings include tags, component state, and attached reference context. Parsed results now expose upstream-style `reference` and `refDate` metadata; parsed components can retain their parse reference; and the configured engine reattaches reference metadata after parser and refiner passes so refiner-created results keep the parse reference on both result and component objects. Reference-array timezone resolution now uses the shared timezone map and custom ambiguous timezone options like upstream `toTimezoneOffset()`, and `Reference` keeps upstream-style `instant` and nullable explicit `timezoneOffset` metadata while exposing timezone-adjusted date/offset helpers corresponding to upstream `ReferenceWithTimezone`, including `getSystemTimezoneAdjustmentMinute()`. `isOnlyTime()`, `isOnlyWeekdayComponent()`, and `isDateWithUnknownYear()` now follow upstream component predicates. Component behavior is not full upstream parity yet. |
| `src/timezone.ts` | `src/Timezone.php` | Partial | The upstream timezone abbreviation map is mirrored, ambiguous timezone offsets are resolved against the parsed/reference date, and custom timezone overrides support fixed and DST-sensitive offsets. DST transition coverage now includes upstream-style ET, CT, MT, PT, and CET ambiguous abbreviation cases. `Timezone::getNthWeekdayOfMonth()` and `Timezone::getLastWeekdayOfMonth()` now mirror upstream exported DST-boundary helpers for custom ambiguous timezones. System timezone coverage now includes upstream-style reference timezone arrays, custom reference timezone aliases, default/missing reference timezone handling, precise `now` references, written datetimes that inherit reference timezones, and explicit-offset datetimes that ignore reference timezone overrides. |
...
Status: behavior-rich but parser coverage remains structurally diverged. English casual date and casual time parsing now use source-shaped `EnCasualDateParser` and `EnCasualTimeParser`. English year-month-day parsing now uses source-shaped `EnYearMonthDayParser`, with upstream `en_year_month_day.test.ts` coverage for numeric separators, strict mode, prefixed matches, month-name forms, casual month/day swapping, and invalid-date rejection. English slash-date parsing now composes the common `SlashDateFormatParser` through `EnSlashDateParser`, while preserving English month-name and weekday-prefixed slash extensions; upstream `en_slash.test.ts` coverage now includes offset month/year matches, prefixed numeric dates, certainty checks, GB little-endian dates, month-name slash dates with attached times and offsets, splitter variants, invalid dates, forward dates, and extra trailing chunks. `EnChrono::createBritishConfiguration()` and `Chrono::gb()` map to upstream `en.GB` by composing `EnSlashDateParser(littleEndian: true)` and `EnMonthNameMiddleEndianParser(shouldSkipYearLikeDate: true)`. English relative-date parsing now uses source-shaped time-unit parser classes for within, casual-relative, ago, later, and relative-date unit expressions; the within, ago, later, and casual-relative parsers compose the shared word-boundary base like upstream while preserving PHP's prefixless within-match trimming and trailing-time casual-relative extension. Numeric slash month/year parsing has been split into `EnSlashMonthFormatParser`; month-only/month-year parsing has been split into `EnMonthNameParser`, with upstream `en_month.test.ts` coverage for month-year forms, exact month-only spans, month-only ranges, `forwardDate`, contextual month expressions, slash month/year, 90s two-digit years, timezone-abbreviation guards, and modal `may` rejection, including upstream `forwardDate` handling, upstream-shaped two-digit `YEAR_PATTERN` handling, shared closest-year helper usage like upstream `findYearClosestToRef`, `parser/ENMonthNameParser` component tags, fallback skipping for rejected `Month Day, Year` dates, and upstream-style rejection of unlikely bare three-letter month abbreviations such as `Mar` while preserving contextual forms such as `in Jan`; common little-endian month-name dates into `EnMonthNameLittleEndianParser`, with upstream `en_month_name_little_endian.test.ts` coverage for explicit years, two-digit years, weekday-prefixed compact dates, parsed weekday-token certainty, prefixed dates, separator variants, ranges, eras, and invalid strict dates; little-endian month date-time forms into `EnMonthNameLittleEndianDateTimeParser`, common middle-endian month-name dates into `EnMonthNameMiddleEndianParser` with upstream `parser/ENMonthNameMiddleEndianParser` tags and `en_month_name_middle_endian.test.ts` coverage for month-only spans, month/year forms, explicit dates, eras, weekday-prefixed dates, ranges, two-digit years, large years, year-like skipping, and invalid dates, trailing-year date-time/range forms into `EnMonthNameTrailingYearParser`, numeric month-name ranges into `EnMonthNameRangeParser`, ordinal-word dates/ranges into `EnMonthNameOrdinalParser`, weekday-prefixed month-name dates into `EnMonthNameWeekdayParser`, single weekday references and weekday ranges into `EnWeekdayParser` plus merge refiners, with split English month-name parsers now sharing closest-year calculation through the common helper. `EnWeekdayParser` now uses `createParsingComponentsAtWeekday()` plus shared weekday calculation and dictionary pattern helpers like upstream, including upstream's implied-noon weekday default while preserving weekend/weekday aliases; `en_weekday.test.ts` coverage now includes exact single-weekday text/index/certainty assertions, last/past/postfix modifiers, this/last/next guessing, weekend/weekday aliases, casual-time merges, overlap merges, ranges, and forward-date ranges. Relative-date reference merging into `EnMergeRelativeFollowByDateRefiner` and `EnMergeRelativeAfterDateRefiner`, explicit year suffix extraction into `EnExtractYearSuffixRefiner` with upstream `en_year.test.ts` coverage for BCE/CE, BC/AD, Buddhist Era, and trailing year suffixes after date/time and range expressions, and common numeric time parsing into `EnTimeExpressionParser` backed by `AbstractTimeExpressionParser`, including upstream-style rejection of unlikely loose numeric time guesses, casual numeric times such as `at 12.30`, meridiem propagation across compact ranges such as `1pm-3`, and overnight range advancement such as `11pm-3`. English date-time and time-followed-by-date merging preserve inferred component certainty and carry component tags, matching upstream `mergeDateTimeComponent`; date/time merging also recalculates ambiguous timezone abbreviation offsets after the actual date is merged, and weekday-only dates followed by time ranges keep implied endpoint dates so `forwardDate` can move both endpoints together. English refiner ordering now mirrors upstream's relative-date prepass, common timezone/weekday wrapper, late second date-time merge, year suffix extraction, and final date-range merge, while retaining PHP-only time-range merge refiners. English relative-date unit coverage now mirrors upstream `en_relative.test.ts` for `this`, `last`, `past`, `next`, `next qtr`, multi-quarter, and `after this year` expressions, including component certainty checks. English integration coverage now includes `en_inter_std.test.ts` ISO/year-month-day cases, including prefixed ISO indexes, fractional timezone offsets, and local timezone uncertainty. English relative-duration coverage now includes exact upstream variants from `en_time_units_ago.test.ts` and `en_time_units_later.test.ts`, including mixed-case half-hour text, abbreviated units, article defaults, nested units, and from-now aliases. English trailing time-range merging now skips a shorter endpoint when a richer parsed endpoint exists at the same position, allowing upstream-style ranges such as `10:30 JST today to 10:30 pst tomorrow` to merge with timezone-aware dates on both endpoints. English date-range merging now implies missing components across endpoints like upstream `AbstractMergeDateRangeRefiner`. English unlikely-format filtering rejects percent-encoded byte fragments that would otherwise be parsed as compact times, matching upstream negative URL cases; English negative-case coverage now also mirrors upstream random-number, hyphen-number, impossible date/time, impossible range, version-number, and invalid numeric slash-date examples. Shared English month and weekday names, pattern helpers, and year parsing now live in `EnConstants`, matching upstream `constants.ts` more closely; split English month and slash parsers now compose those pattern helpers instead of rebuilding dictionary alternations locally. The older grouped English time parser has been removed.
...
Status: partial and structurally mixed. Spanish casual-date coverage now includes upstream index/text/component assertions, random evening/noon/midnight cases, and upstream negative boundary examples, with PHP byte-offset indexing documented by tests for accented prefixes. Spanish slash-date coverage now includes upstream text/index/component assertions for weekday-prefixed little-endian numeric dates. Spanish month-name coverage now includes upstream explicit dates, BCE/CE eras, uppercase compact weekday/month forms, accented weekday prefixes with PHP byte-offset assertions, `de` date forms, ranges, and date-time merges from `es_month_name_little_endian.test.ts`. Spanish time-expression coverage now includes upstream text/index/component/certainty assertions for single times, ranges, meridiem-implied ranges, date-time merges with PHP byte-offset assertions for accented prefixes, and random expression text preservation from `es_time_exp.test.ts`. Spanish within-duration coverage now includes upstream text/index/component certainty assertions for day, minute, second, week, month, and year durations, including parser tags from `ESTimeUnitWithinFormatParser`. Future Spanish work should split grouped parser classes toward the upstream parser family before expanding behavior.
...
Status: partial. French casual-date coverage now includes upstream index/text/component assertions for now/today/tomorrow/yesterday/day-period cases, combined date-time cases, and day-period meridiem expectations, with PHP byte-offset indexing documented by tests for accented prefixes. Split French month/time behavior toward upstream parser classes and move timezone extraction methods fully into common refiners before adding broader French coverage.
...
Status: source-shaped parser/refiner coverage now exists for all named upstream German parser/refiner files, with legacy grouped classes kept as fallbacks for behavior not yet fully migrated. German casual-date coverage now includes upstream index/text/component assertions for now/today/tomorrow/yesterday/day-period cases, combined date-time cases, day-period meridiem expectations, and upstream negative boundary examples; bare casual date words preserve the reference clock as implied time like upstream. German weekday coverage now includes upstream index/text/component/certainty assertions for bare weekdays, modifiers, postmodifier `nächste Woche` forms, forward-date weekday ranges, and weekday/date overlap merges. German within-duration coverage now includes upstream index/text/component/certainty assertions for numeric, spelled-out, abbreviation, hour/minute/second/week/month/year, and `für` forms, with PHP byte-offset indexing documented by tests for multibyte prefixes. German time-expression coverage now includes upstream component/certainty assertions for simple times, day-period meridiems, range endpoints, timezone absence/presence, CET/CEST offsets, and false-positive abbreviation guards. German little-endian month-name coverage now includes upstream index/text/component assertions for explicit years, BCE/CE era suffixes, weekday-prefixed dates, contextual `am` dates, same-month ranges, cross-month ranges, explicit end-year ranges, and Austrian month spelling.
...
Status: source-shaped parser/refiner coverage now exists for all named upstream Finnish parser/refiner files plus common ISO/slash parser composition. Finnish coverage now includes upstream casual date/time component assertions, weekday single/modifier/variation/abbreviation assertions, `klo`/`kello` time and range assertions, date-time merging, and time-unit `sisällä`/`kuluessa`/`päästä` examples with source-shaped text/index/component expectations. Continue Finnish by comparing behavior against upstream tests and tightening edge cases.
...
Status: source-shaped parser/refiner coverage now exists for all named upstream Dutch parser/refiner files. Dutch month-name coverage now includes upstream uppercase weekday/month abbreviations, abbreviated weekday prefixes, ordinal dates, `tot` same-month ranges, cross-month ranges without explicit years, date-time merges, and two-digit year variants from `nl_month_name_little_endian.test.ts`. Dutch slash-date coverage now includes upstream splitter variants (`YYYY-MM-DD`, `YYYY/MM/DD`, `YYYY.MM.DD`, `DD-MM-YYYY`, `DD/MM/YYYY`, and `DD.MM.YYYY`) plus impossible-date rejection cases. Dutch time-expression parsing now consumes upstream casual meridiem suffixes such as `vanavond`, `'s avonds`, `'s ochtends`, and `in de namiddag` as part of the numeric time result, applying the expected AM/PM certainty. Dutch casual-date/time coverage now includes upstream casual date ranges, casual time implication certainty for ranges, casual phrases followed by explicit numeric times, and negative boundary cases from `nl_casual.test.ts`. Dutch relative time-unit coverage now includes upstream strict later expressions, within-duration certainty by unit granularity, timezone-offset certainty for time-level relative durations, within/from-now/ago spelling variants, nested ago durations, spelled-out seconds, Dutch `binnen de` and spelled-number within forms, compact units such as `1u`/`+15min`/`-3jr`, compound signed and upcoming/past durations, and default-English negative cases from the upstream Dutch ago tests. Continue Dutch by comparing behavior against upstream tests and tightening edge cases.
...
Status: initial source-shaped parser coverage exists for all named upstream Swedish parser files plus common ISO/slash parser composition. Swedish casual-date coverage now includes upstream component/certainty assertions for today/tomorrow/yesterday/before-yesterday and day-period phrases. Swedish month-name coverage now includes upstream index/text/component/certainty assertions for single expressions, abbreviated months, inferred versus explicit years, same-month ranges, and impossible-date cases. Swedish weekday coverage now includes upstream bare/prefixed weekdays, next/last modifiers, weekday variation cases, and component certainty assertions for implied dates versus certain weekdays. Swedish casual-relative coverage now includes upstream positive/negative modifier phrases, number-word units, compact signed units, explicit signed forms such as `+15 minuter`, `+1min`, and `-3år`, and the upstream parser constructor option for disabling abbreviated time units. Continue Swedish by comparing behavior against upstream tests and tightening edge cases.
...
Status: initial source-shaped parser/refiner coverage exists for all named upstream Portuguese parser/refiner files plus common slash parser composition. Portuguese casual-date and month-name coverage now includes upstream index/text/component assertions, with PHP byte-offset indexing documented by tests for multibyte prefixes. Portuguese time-expression coverage now includes upstream single/range/date-time/meridiem-certainty cases and the random date-time text-preservation examples from `pt_time_exp.test.ts`, including weekday-prefixed compact times and compact ranges. Continue Portuguese by comparing behavior against upstream tests and tightening edge cases.
...
Status: initial source-shaped parser/refiner coverage exists for all named upstream Japanese parser/refiner files. Japanese slash-date coverage now includes upstream index/text/component assertions for single dates and slash ranges. Japanese weekday coverage now includes upstream component/certainty assertions for bare weekdays, previous-weekday modifiers, parenthesized weekdays, and forward weekday ranges. Japanese time-expression coverage now includes upstream-style component, meridiem, and certainty assertions for single times, Japanese-numeral ranges, explicit meridiem ranges, and date-times with seconds. Japanese date-followed-by-weekday coverage now mirrors upstream text/component assertions for month-day, slash-date, full-width slash-date, and full-width date-time range combinations. Continue Japanese by comparing behavior against upstream tests and tightening edge cases.
...
Status: initial source-shaped parser/refiner coverage exists for all named upstream Chinese parser/refiner files. Default `zh` engine coverage now mirrors upstream `zh.test.ts` for source-common ISO parsing and combined Simplified/Traditional casual date-time input such as `明天早上8点` and `明天早上8點`. Continue Chinese by comparing behavior against upstream tests and tightening edge cases.
...
4. Continue splitting remaining grouped parser classes into upstream-shaped parser classes, prioritizing locales where grouped extensions still own behavior absent from the upstream parser family.
