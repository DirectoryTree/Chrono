...
class FrTimeUnitAgoFormatParser extends AbstractParserWithWordBoundary
{
    use InteractsWithFrenchRelativeDates;

    /**
     * Get the French past relative time-unit parser pattern.
     */
    protected function innerPattern(Reference $reference, Options $options): string
    {
        return 'il\s+y\s+a\s*(?<duration>'.$this->durationPattern().')(?=\W|$)';
    }
    ...
    protected function innerExtract(array $match, Reference $reference, Options $options): ?ParsedComponents
    {
        ...
        return ParsedComponents::createRelativeFromReference($reference, Duration::reverse($duration))
            ->addTag('parser/FRTimeUnitAgoFormatParser');
    }
