<?php

namespace Chrono;

use Carbon\CarbonImmutable;

class Timezone
{
    /**
     * @var array<string, int|string>
     */
    protected const ABBREVIATIONS = [
        'ACDT' => 630,
        'ACST' => 570,
        'ADT' => -180,
        'AEDT' => 660,
        'AEST' => 600,
        'AFT' => 270,
        'AKDT' => -480,
        'AKST' => -540,
        'ALMT' => 360,
        'AMST' => -180,
        'AMT' => -240,
        'ANAST' => 720,
        'ANAT' => 720,
        'AQTT' => 300,
        'ART' => -180,
        'AST' => -240,
        'AWDT' => 540,
        'AWST' => 480,
        'AZOST' => 0,
        'AZOT' => -60,
        'AZST' => 300,
        'AZT' => 240,
        'BNT' => 480,
        'BOT' => -240,
        'BRST' => -120,
        'BRT' => -180,
        'BST' => 60,
        'BTT' => 360,
        'CAST' => 480,
        'CAT' => 120,
        'CCT' => 390,
        'CDT' => -300,
        'CEST' => 120,
        'CET' => 'CET',
        'CHADT' => 825,
        'CHAST' => 765,
        'CKT' => -600,
        'CLST' => -180,
        'CLT' => -240,
        'COT' => -300,
        'CST' => -360,
        'CT' => 'CT',
        'CVT' => -60,
        'CXT' => 420,
        'CHST' => 600,
        'DAVT' => 420,
        'EASST' => -300,
        'EAST' => -360,
        'EAT' => 180,
        'ECT' => -300,
        'EDT' => -240,
        'EEST' => 180,
        'EET' => 120,
        'EGST' => 0,
        'EGT' => -60,
        'EST' => -300,
        'ET' => 'ET',
        'FJST' => 780,
        'FJT' => 720,
        'FKST' => -180,
        'FKT' => -240,
        'FNT' => -120,
        'GALT' => -360,
        'GAMT' => -540,
        'GET' => 240,
        'GFT' => -180,
        'GILT' => 720,
        'GMT' => 0,
        'GST' => 240,
        'GYT' => -240,
        'HAA' => -180,
        'HAC' => -300,
        'HADT' => -540,
        'HAE' => -240,
        'HAP' => -420,
        'HAR' => -360,
        'HAST' => -600,
        'HAT' => -90,
        'HAY' => -480,
        'HKT' => 480,
        'HLV' => -210,
        'HNA' => -240,
        'HNC' => -360,
        'HNE' => -300,
        'HNP' => -480,
        'HNR' => -420,
        'HNT' => -150,
        'HNY' => -540,
        'HOVT' => 420,
        'ICT' => 420,
        'IDT' => 180,
        'IOT' => 360,
        'IRDT' => 270,
        'IRKST' => 540,
        'IRKT' => 540,
        'IRST' => 210,
        'IST' => 330,
        'JST' => 540,
        'KGT' => 360,
        'KRAST' => 480,
        'KRAT' => 480,
        'KST' => 540,
        'KUYT' => 240,
        'LHDT' => 660,
        'LHST' => 630,
        'LINT' => 840,
        'MAGST' => 720,
        'MAGT' => 720,
        'MART' => -510,
        'MAWT' => 300,
        'MDT' => -360,
        'MESZ' => 120,
        'MEZ' => 60,
        'MHT' => 720,
        'MMT' => 390,
        'MSD' => 240,
        'MSK' => 180,
        'MST' => -420,
        'MT' => 'MT',
        'MUT' => 240,
        'MVT' => 300,
        'MYT' => 480,
        'NCT' => 660,
        'NDT' => -90,
        'NFT' => 690,
        'NOVST' => 420,
        'NOVT' => 360,
        'NPT' => 345,
        'NST' => -150,
        'NUT' => -660,
        'NZDT' => 780,
        'NZST' => 720,
        'OMSST' => 420,
        'OMST' => 420,
        'PDT' => -420,
        'PET' => -300,
        'PETST' => 720,
        'PETT' => 720,
        'PGT' => 600,
        'PHOT' => 780,
        'PHT' => 480,
        'PKT' => 300,
        'PMDT' => -120,
        'PMST' => -180,
        'PONT' => 660,
        'PST' => -480,
        'PT' => 'PT',
        'PWT' => 540,
        'PYST' => -180,
        'PYT' => -240,
        'RET' => 240,
        'SAMT' => 240,
        'SAST' => 120,
        'SBT' => 660,
        'SCT' => 240,
        'SGT' => 480,
        'SRT' => -180,
        'SST' => -660,
        'TAHT' => -600,
        'TFT' => 300,
        'TJT' => 300,
        'TKT' => 780,
        'TLT' => 540,
        'TMT' => 300,
        'TVT' => 720,
        'ULAT' => 480,
        'UTC' => 0,
        'UYST' => -120,
        'UYT' => -180,
        'UZT' => 300,
        'VET' => -210,
        'VLAST' => 660,
        'VLAT' => 660,
        'VUT' => 660,
        'WAST' => 120,
        'WAT' => 60,
        'WEST' => 60,
        'WESZ' => 60,
        'WET' => 0,
        'WEZ' => 0,
        'WFT' => 720,
        'WGST' => -120,
        'WGT' => -180,
        'WIB' => 420,
        'WIT' => 540,
        'WITA' => 480,
        'WST' => 780,
        'WT' => 0,
        'YAKST' => 600,
        'YAKT' => 600,
        'YAPT' => 600,
        'YEKST' => 360,
        'YEKT' => 360,
    ];

    /**
     * Resolve a timezone abbreviation to an offset in minutes.
     */
    public static function offset(string $abbr, CarbonImmutable $date, ?Options $options = null): ?int
    {
        $abbr = strtoupper($abbr);
        $custom = $options?->timezones()[$abbr] ?? null;

        if (is_int($custom)) {
            return $custom;
        }

        if (is_array($custom)) {
            return self::customAmbiguousOffset($custom, $date);
        }

        if (! array_key_exists($abbr, self::ABBREVIATIONS)) {
            return null;
        }

        $offset = self::ABBREVIATIONS[$abbr];

        return is_string($offset) ? self::ambiguousOffset($offset, $date) : $offset;
    }

    /**
     * @param  array<string, mixed>  $timezone
     */
    protected static function customAmbiguousOffset(array $timezone, CarbonImmutable $date): ?int
    {
        if (! isset($timezone['timezoneOffsetDuringDst'], $timezone['timezoneOffsetNonDst'])) {
            return null;
        }

        $start = isset($timezone['dstStart']) && is_callable($timezone['dstStart'])
            ? $timezone['dstStart']($date->year)
            : null;
        $end = isset($timezone['dstEnd']) && is_callable($timezone['dstEnd'])
            ? $timezone['dstEnd']($date->year)
            : null;

        if (! $start instanceof CarbonImmutable || ! $end instanceof CarbonImmutable) {
            return (int) $timezone['timezoneOffsetNonDst'];
        }

        return ($date->greaterThan($start) && ! $date->greaterThan($end))
            ? (int) $timezone['timezoneOffsetDuringDst']
            : (int) $timezone['timezoneOffsetNonDst'];
    }

    /**
     * Resolve the timezone offset.
     */
    protected static function ambiguousOffset(string $timezone, CarbonImmutable $date): ?int
    {
        return match ($timezone) {
            'ET' => self::isNorthAmericanDst($date) ? -240 : -300,
            'CT' => self::isNorthAmericanDst($date) ? -300 : -360,
            'MT' => self::isNorthAmericanDst($date) ? -360 : -420,
            'PT' => self::isNorthAmericanDst($date) ? -420 : -480,
            'CET' => self::isCentralEuropeanDst($date) ? 120 : 60,
            default => null,
        };
    }

    /**
     * Determine whether the date is in North American daylight saving time.
     */
    protected static function isNorthAmericanDst(CarbonImmutable $date): bool
    {
        $start = self::getNthWeekdayOfMonth($date->year, Month::MARCH, Weekday::SUNDAY, 2, 2);
        $end = self::getNthWeekdayOfMonth($date->year, Month::NOVEMBER, Weekday::SUNDAY, 1, 2);

        return $date->greaterThan($start) && ! $date->greaterThan($end);
    }

    /**
     * Determine whether the date is in Central European daylight saving time.
     */
    protected static function isCentralEuropeanDst(CarbonImmutable $date): bool
    {
        $start = self::getLastWeekdayOfMonth($date->year, Month::MARCH, Weekday::SUNDAY, 2);
        $end = self::getLastWeekdayOfMonth($date->year, Month::OCTOBER, Weekday::SUNDAY, 3);

        return $date->greaterThan($start) && ! $date->greaterThan($end);
    }

    /**
     * Get the date of the nth occurrence of a weekday in a month.
     */
    public static function getNthWeekdayOfMonth(int $year, Month|int $month, Weekday|int $weekday, int $nth, int $hour = 0): CarbonImmutable
    {
        $date = CarbonImmutable::create($year, self::monthValue($month), 1, $hour);
        $seen = 0;

        while (true) {
            if ($date->dayOfWeek === self::weekdayValue($weekday)) {
                $seen++;

                if ($seen === $nth) {
                    return $date;
                }
            }

            $date = $date->addDay();
        }
    }

    /**
     * Get the date of the last occurrence of a weekday in a month.
     */
    public static function getLastWeekdayOfMonth(int $year, Month|int $month, Weekday|int $weekday, int $hour = 0): CarbonImmutable
    {
        $date = CarbonImmutable::create($year, self::monthValue($month), 1, $hour)->endOfMonth()->startOfDay()->hour($hour);

        while ($date->dayOfWeek !== self::weekdayValue($weekday)) {
            $date = $date->subDay();
        }

        return $date;
    }

    /**
     * Resolve the month value.
     */
    protected static function monthValue(Month|int $month): int
    {
        return $month instanceof Month ? $month->value : $month;
    }

    /**
     * Resolve the weekday value.
     */
    protected static function weekdayValue(Weekday|int $weekday): int
    {
        return $weekday instanceof Weekday ? $weekday->value : $weekday;
    }
}
